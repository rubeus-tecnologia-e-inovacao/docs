...
| `tipoVinculo` | `string` | Não |  Cadastrar o tipo do vinculo, pessoa ou oportunidade |
| `tempoNotificacao` | `integer` | Não | Tempo para notificar a atividade por e-mail de acordo com o tipoTempo |
| `tipoTempo` | `integer` | Não | Cadastrar o tipo do tempo <br> **Tabela de tipo** <br> 1 - minutos <br> 2 - horas <br> 3 - dias|
| `pessoas` | `array[] of integer` | Não | Identificação de pessoas |
| `agendamentoUnico` | `string` | Não |  Sempre enviar por padrão `true` |
| `oportunidades` | `array[] of integer` | Não | Idenficação de oportunidades |
| `razaoOportunidade` | `integer` | Não | Identificação resumo atual da oportunidade |
| `responsavelUnico` | `integer` | Não | Identificação do responsável |
| `assinatura` | `integer` | Não | Identificação da assinatura | 
| `enviarSemAssinatura` | `integer` | Não | Usado para concluir atividade mesmo sem uma assinatura. |
| `baseLegalEnvioSemAssinatura` | `integer` | Não | Identificação da base legal utilizada para concluir atividade sem assinatura. [Listagem base legal](/api_crm/metodosdelistagem/#listar-bases-legais) |
| `justificativaEnvioSemAssinatura` | `string` | Não | Justificativa para concluir atividade sem assinatura |
...
!!! done ""
    ...
    <strong class='REST POST'>POST</strong><strong class="MIME">application/json</strong> /api/Atividade/listarAtividades
...
| `colunaPesquisa` | `string` | Não | Nome referente à informação a ser pesquisada no campo de pesquisa. <br><br>** Atributos <br><br> 'nomePessoa', 'pessoaEmail', 'pessoaTelefone', 'processo', 'nomeOferta', 'tituloAtividade', 'responsavelAtividade', 'vencimentoAtividade', 'momentoConclusaoAtividade', 'momentoCriacaoAtividade', 'idRegistro' ** |
...
| `comunicacaoContato` | `integer` | Não | Comunicação do contato <br> 1 - Não Comunicável <br> 2- Comunicável |
| `origem` | `integer` | Sim | Código de identificação do [canal](/api_crm/apresentacao/#autenticacao). |
| `token` | `string` | Sim | Chave de acesso única referente ao canal. |
