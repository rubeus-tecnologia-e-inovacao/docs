...
| `codOferta` | `string` | Não | Código de identificação externa da oferta do curso.<br>**O código da oferta do curso é obrigatório caso se deseje vincular um curso em um registro na Plataforma Rubeus, caso o `curso` não seja enviado.<br>O canal (origem) do evento deve ser o mesmo da oferta e do curso enviado.** | 
...
| **↳** `dadosOportunidade.codRegistro` | `string` | <rb-tooltip text="Informação obrigatória caso seja enviado o `dadosOportunidade`">Condicional </rb-tooltip> | Caso o registro que se deseja alterar não possúa `codOferta` ou `codCurso`, é possível utilizar o `codRegistro` para altera-lo | 
| **↳** `dadosOportunidade.codOferta` | `string` | <rb-tooltip text="Informação obrigatória caso seja enviado o `dadosOportunidade`">Condicional </rb-tooltip> | Caso o registro que se deseja alterar não possúa `codRegistro` ou `codCurso`, é possível utilizar o `codOferta` para altera-lo | 
| **↳** `dadosOportunidade.codCurso` | `string` | <rb-tooltip text="Informação obrigatória caso seja enviado o `dadosOportunidade`">Condicional </rb-tooltip> | Caso o registro que se deseja alterar não possúa `codOferta` ou `codRegistro`, é possível utilizar o `codCurso` para altera-lo | 
