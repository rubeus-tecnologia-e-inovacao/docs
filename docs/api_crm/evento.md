...
| **↳** `dadosOportunidade.codRegistro` | `string` | <rb-tooltip text="Informação obrigatória caso seja enviado o `dadosOportunidade`">Condicional </rb-tooltip> | Caso o registro que se deseja alterar não possua `codOferta` ou `codCurso`, é possível utilizar o `codRegistro` para altera-lo | 
| **↳** `dadosOportunidade.codOferta` | `string` | <rb-tooltip text="Informação obrigatória caso seja enviado o `dadosOportunidade`">Condicional </rb-tooltip> | Caso o registro que se deseja alterar não possua `codRegistro` ou `codCurso`, é possível utilizar o `codOferta` para altera-lo | 
| **↳** `dadosOportunidade.codCurso` | `string` | <rb-tooltip text="Informação obrigatória caso seja enviado o `dadosOportunidade`">Condicional </rb-tooltip> | Caso o registro que se deseja alterar não possua `codOferta` ou `codRegistro`, é possível utilizar o `codCurso` para altera-lo | 
